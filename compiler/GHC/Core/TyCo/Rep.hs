...
import GHC.Types.Unique.Supply (mkSplitUniqSupply, takeUniqFromSupply, UniqSupply)
...
uniqSupply = unsafePerformIO $ mkSplitUniqSupply 'M'

{-# NOINLINE templateMatchabilityVar #-}
templateMatchabilityVar :: TyVar
templateMatchabilityVar = unsafePerformIO $ do
  let (uniq, _) = takeUniqFromSupply uniqSupply
  return $ mkTyVar (mkSystemName uniq (mkTyVarOcc "n")) matchabilityTy
...
-- defaultMatchability _ = mkTyVarTy templateMatchabilityVar
