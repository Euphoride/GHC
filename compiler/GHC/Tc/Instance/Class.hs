...
module GHC.Tc.Instance.Class (
     ...
     isMatchableTyCon,
     mkMatchableInst,
     emitUnifiesMatchable,
     emitUnifiesUnmatchable,
     getAllMatchabilityTyVars,
     annotateTyCon,
  ) where
...
import GHC.Tc.Types.Origin ( InstanceWhat (..), SafeOverlapping, CtOrigin(GetFieldOrigin, GivenOrigin), SkolemInfoAnon( UnkSkol, RuntimeUnkSkol ))
...
import {-# SOURCE #-} GHC.Tc.Solver.Monad (emitNewGivens, runTcS)
...
getAllMatchabilityTyVars :: Type -> TcM [Type]
getAllMatchabilityTyVars = go
  where
    go (FunTy _ _ m arg res) = do
      arg_mvs <- go arg
      res_mvs <- go res
      return (m : arg_mvs ++ res_mvs)
...
emitUnifiesUnmatchable :: Type -> TcM ()
emitUnifiesUnmatchable m = do  
  let coercion = mkUnivCo (PluginProv "Matchability") [] Nominal m unmatchableDataConTy
  
  loc <- getCtLocM (GivenOrigin RuntimeUnkSkol) Nothing
  _ <- runTcS $ emitNewGivens loc [(Nominal, coercion)]
  return ()

emitUnifiesMatchable :: Type -> TcM ()
emitUnifiesMatchable m = do  
  let coercion = mkUnivCo (PluginProv "Matchability") [] Nominal m matchableDataConTy
...
annotateTyCon tc = do
  mvars <- getAllMatchabilityTyVars tc_kind
  whenM (return $ isMatchableTyCon tc Nominal) $ mapM_ emitUnifiesMatchable mvars
  whenM (return $ not $ isMatchableTyCon tc Nominal) $ mapM_ emitUnifiesUnmatchable mvars
  where 
