  ...
  = do { mkTrCon <- dsLookupGlobalId mkTrConName
       ...
       ; let ks = tyConAppArgs ty
             ...
             toSomeTypeRep t ev = do
                 rep <- getRep ev t
                 return $ mkCoreConApps someTypeRepDataCon [Type (typeKind t), Type t, matchableEv matchableDataCon t, rep]
...
ds_ev_typeable ty (EvTypeableTyApp ev1 ev2)
  ...
  = do { e1  <- getRep ev1 t1
       ...
       ; let expr =  mkApps (mkTyApps (Var mkTrApp) [ k1, k2, t1, t2 ])
                            [ matchableEv matchableDataCon t1 , matchableEv matchableDataCon t2, e1, e2 ]
